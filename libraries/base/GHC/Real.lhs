...
-- import GHC.Exception (throw, ArithException(RatioZeroDenominator))
...
\begin{code}
{-# NOINLINE reduce #-}--FHW this removed empty case alternatives associated with Exceptions
...
class  (Real a, Fractional a) => RealFrac a  where
    ...
    {-# NOINLINE truncate #-}--FHW this removed empty case alternatives associated with Exceptions
...
instance  Integral Int  where
    ...
    {-# NOINLINE div #-} --FHW this removed empty case alternatives associated with Exceptions
    a `div` b
     | b == 0                     = divZeroError
     | b == (-1) && a == minBound = overflowError -- Note [Order of tests]
                                                  -- in GHC.Int
...
instance  (Integral a)  => Fractional (Ratio a)  where
    {-# NOINLINE recip #-}--FHW this removed empty case alternatives associated with Exceptions
    (x:%y) / (x':%y')   =  (x*y') % (y*x')
...
{-# RULES "(^)/Rational"    (^) = (^%^) #-}
{-# NOINLINE (^%^) #-}--FHW this removed empty case alternatives associated with Exceptions
...
{-# NOINLINE lcm #-}--FHW this removed empty case alternatives associated with Exceptions
lcm _ 0         =  0
lcm 0 _         =  0
