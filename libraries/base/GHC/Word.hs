...
--import GHC.Show
--import GHC.Err
--import GHC.Float ()     -- for RealFrac methods -- FHW
...
{- FHW
        | y /= 0                  = W8# (x# `remWord#` y#)
        | otherwise               = divZeroError -}
    quotRem (W8# x#) (W8# y#)= case x# `quotRemWord#` y# of
                                    (# q, r #) ->
                                        (W8# q, W8# r)
{- FHW
...
{- FHW
        | y /= 0                    = W16# (x# `remWord#` y#)
        | otherwise                 = divZeroError -}
    quotRem (W16# x#) (W16# y#)  = case x# `quotRemWord#` y# of
                                    (# q, r #) ->
                                        (W16# q, W16# r)
{- FHW
        | y /= 0                  = case x# `quotRemWord#` y# of
...
instance Integral Word32 where
    quot    (W32# x#) (W32# y#)  = W32# (x# `quotWord#` y#)
{- FHW
        | y /= 0                    = W32# (x# `quotWord#` y#)
        | otherwise                 = divZeroError -}
    rem     (W32# x#) (W32# y#)  = W32# (x# `remWord#` y#)
{- FHW
        | y /= 0                    = W32# (x# `remWord#` y#)
        | otherwise                 = divZeroError -}
    div     (W32# x#) (W32# y#)   = W32# (x# `quotWord#` y#)
{- FHW
        | y /= 0                    = W32# (x# `quotWord#` y#)
        | otherwise                 = divZeroError -}
    mod     (W32# x#) (W32# y#)   = W32# (x# `remWord#` y#)
{- FHW
        | y /= 0                    = W32# (x# `remWord#` y#)
        | otherwise                 = divZeroError -}
    quotRem (W32# x#) (W32# y#) = case x# `quotRemWord#` y# of
                                    (# q, r #) ->
                                        (W32# q, W32# r)
{- FHW
        | y /= 0                  = case x# `quotRemWord#` y# of
                                    (# q, r #) ->
                                        (W32# q, W32# r)
        | otherwise                 = divZeroError -}
    divMod  (W32# x#) (W32# y#)   = (W32# (x# `quotWord#` y#), W32# (x# `remWord#` y#))
{- FHW
...
{- FHW
        | x /= minBound = x - 1
        | otherwise     = predError "Word64" -}
    toEnum (I# i#) = W64# (int2Word# i#)
{- FHW
        | i >= 0        = W64# (int2Word# i#)
        | otherwise     = toEnumError "Word64" i (minBound::Word64, maxBound::Word64) -}
    fromEnum (W64# x#)= I# (word2Int# x#)
{- FHW
        | x <= fromIntegral (maxBound::Int)
                        = I# (word2Int# x#)
        | otherwise     = fromEnumError "Word64" x -}
...
{- FHW
        | y /= 0                    = W64# (x# `remWord#` y#)
        | otherwise                 = divZeroError -}
    quotRem (W64# x#) (W64# y#) = case x# `quotRemWord#` y# of
                                    (# q, r #) ->
                                        (W64# q, W64# r)
{- FHW
        | y /= 0                  = case x# `quotRemWord#` y# of
