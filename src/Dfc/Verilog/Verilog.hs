...
verilog moduleName (SDataflow tdefs nodes') =
    typeDefs tei ++
 ...
 where
   ...
   typeDict = V.Comment $ "TYPE_START\n" ++ typeLines ++ 
    ...
    where
      ...
      mkTypeLine tc pSize = intercalate " " [verilatify tc,
                        show pSize,
                        show (tagBitsOf tei tc),
                        intercalate " " (map showTup $ variantTuples tei tc) ]
   ...
   synthesize (SNode "mux" _ [Bind select sType , Group inputBinds]
                                  [Bind output dType]) =
     [ V.Reg (muxed, V.Unsigned (dataBits + 1))
     , V.Reg (onehot, V.Unsigned numInputs)
     ...
     , V.Assign (V.LConcat $ reverse $ map (V.Var . ready) inputs) $
        ...
        where    
          ...
          assignSelected expr =
             V.Blocking (V.LConcat [V.Var onehot, V.Var muxed]) expr
...
--      else []) ++
      [ V.Reg (onehot, V.Unsigned numOutputs)
...
-}
   ...
   synthesize (SNode "merge" _ [Group inputBinds] [Bind output typ]) =
     ...
     where
       ...
       selectedDecl = V.Reg (selected_name, V.Unsigned numInputs)
       ...
       selectDecl = V.Reg (select_name, V.Unsigned numInputs)
       ...
       selectUpdate = V.AlwaysSequential $ V.NonBlocking select $
   ...
   synthesize (SNode "mergectrl" [_,_] [Group inputBinds]
    ...
    where
      ...
      (priorityEncoder, select_d) =
          combAssign (nodeName ++ "_select_d", V.Unsigned numInputs) $
      ...
      (selectState, select_q) =
          seqAssign (nodeName ++ "_select_q", V.Unsigned numInputs)
...
--    node.  Selector is effectively a "go" signal.
   ...
   synthesize (SNode "bram" _ [Bind input iType] [Bind output _]) =
       ...
       , V.Reg (address, V.Unsigned addressBits)
       , V.Reg (din, dataType)
       , V.Reg (q, dataType)
       ] ++
       map (\n -> V.Reg (n, V.Boolean))
       ...
       , V.AlwaysSequential $ V.ifThen (readye input) $ V.SeqBlock []
...
-}
   ...
   synthesizeMultiOutputs nodeName readySignal validExpr outputs =
    ...
    where
      ...
      emittedDecl = V.Reg (emitted_name, V.Unsigned numOutputs)
      ...
      doneDecl = V.Reg (done_name, V.Unsigned numOutputs)
   ...
   dbuf input output initExpr =
       ...
       , V.AlwaysSequential $ V.ifThen (readye input) $
                            V.NonBlocking bufData (datae input)
       ]
      where
...
vbin op e1 e2 = V.BinOp e1 op e2
