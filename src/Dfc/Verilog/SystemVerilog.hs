...
Keep this as DFC-agnostic as possible, i.e., any magic names or
special constructs should be generated elsewhere.

-}

module Dfc.Verilog.SystemVerilog where
...
data Type = Boolean
          ...
          | Signed Integer    -- Bit vector of given width
          | Unsigned Integer  -- Bit vector of given width
          | Const Type    -- Constant
          | Enum [String] -- Enumerated type
          | StructUnion StructUnion Packed [(Type,String)] -- Aggregate
          | Array Type Integer -- Array of the given size
...
downToZero :: Integer -> Doc
downToZero w = brackets $ integer (w - 1) <> colon <> char '0'
...
instance PP Expr where
  ...
  pp (Sized size value) = (if value < 0 then char '-' else empty) <>
     int size <> char '\'' <> char 'd' <> integer (fromInteger $ abs value)
